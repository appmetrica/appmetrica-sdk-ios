...
describe(@"AMAKeychainTests", ^{
    ...
    context(@"with default identifier", ^{
        ...
        context(@"Errors", ^{
            ...
            NSError *(^errorWithCode)(kAMAKeychainErrorCode, OSStatus) = ^(kAMAKeychainErrorCode code, OSStatus status) {
            ...
            context(@"Copy error", ^{
                ...
                context(@"Getter", ^{
                    ...
                    it(@"Should fill valid error", ^{
                        ...
                        [[error should] equal:errorWithCode(kAMAKeychainErrorCodeGet, errorStatus)];
                    });
                });
                context(@"Setter", ^{
                    it(@"Should add key", ^{
