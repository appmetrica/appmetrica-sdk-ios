...
NS_ASSUME_NONNULL_BEGIN
    ...
    #define AMAFMDBAutorelease(__v) ([__v autorelease]);
    #define AMAFMDBReturnAutoreleased AMAFMDBAutorelease

    #define AMAFMDBRetain(__v) ([__v retain]);
    #define AMAFMDBReturnRetained AMAFMDBRetain

    #define AMAFMDBRelease(__v) ([__v release]);

    #define AMAFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define AMAFMDBAutorelease(__v)
    #define AMAFMDBReturnAutoreleased(__v) (__v)

    #define AMAFMDBRetain(__v)
    #define AMAFMDBReturnRetained(__v) (__v)

    #define AMAFMDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define AMAFMDBDispatchQueueRelease(__v)
    #else
        #define AMAFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
 ...
 */
typedef int(^AMAFMDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);

/**
 Enumeration used in checkpoint methods.
 */
typedef NS_ENUM(int, AMAFMDBCheckpointMode) {
    AMAFMDBCheckpointModePassive  = 0, // SQLITE_CHECKPOINT_PASSIVE,
    AMAFMDBCheckpointModeFull     = 1, // SQLITE_CHECKPOINT_FULL,
    AMAFMDBCheckpointModeRestart  = 2, // SQLITE_CHECKPOINT_RESTART,
    AMAFMDBCheckpointModeTruncate = 3  // SQLITE_CHECKPOINT_TRUNCATE
};
...
@interface AMAFMDatabase : NSObject
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) AMAFMDBExecuteStatementsCallbackBlock _Nullable)block;
...
- (AMAFMResultSet * _Nullable)executeQuery:(NSString*)sql, ...;
...
- (AMAFMResultSet * _Nullable)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (AMAFMResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (AMAFMResultSet * _Nullable)executeQuery:(NSString *)sql values:(NSArray * _Nullable)values error:(NSError * _Nullable __autoreleasing *)error;
...
- (AMAFMResultSet * _Nullable)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary * _Nullable)arguments;


// Documentation forthcoming.
- (AMAFMResultSet * _Nullable)executeQuery:(NSString *)sql withVAList:(va_list)args;
 ...
 */
- (BOOL)checkpoint:(AMAFMDBCheckpointMode)checkpointMode error:(NSError * _Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return @c YES on success, otherwise @c NO .
 */
- (BOOL)checkpoint:(AMAFMDBCheckpointMode)checkpointMode name:(NSString * _Nullable)name error:(NSError * _Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return @c YES on success, otherwise @c NO .
 */
- (BOOL)checkpoint:(AMAFMDBCheckpointMode)checkpointMode name:(NSString * _Nullable)name logFrameCount:(int * _Nullable)logFrameCount checkpointCount:(int * _Nullable)checkpointCount error:(NSError * _Nullable *)error;
...
- (AMASqliteValueType)valueType:(void *)argv;
...
@interface AMAFMStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
