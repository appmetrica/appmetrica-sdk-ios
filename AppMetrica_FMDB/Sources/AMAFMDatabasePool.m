...
- (void)pushDatabaseBackInPool:(AMAFMDatabase*)db;
- (AMAFMDatabase*)db;

@end


@implementation AMAFMDatabasePool
...
+ (instancetype)databasePoolWithURL:(NSURL *)url {
    return AMAFMDBReturnAutoreleased([[self alloc] initWithPath:url.path]);
}

+ (instancetype)databasePoolWithPath:(NSString *)aPath flags:(int)openFlags {
    return AMAFMDBReturnAutoreleased([[self alloc] initWithPath:aPath flags:openFlags]);
}

+ (instancetype)databasePoolWithURL:(NSURL *)url flags:(int)openFlags {
    return AMAFMDBReturnAutoreleased([[self alloc] initWithPath:url.path flags:openFlags]);
}
...
- (instancetype)initWithPath:(NSString*)aPath flags:(int)openFlags vfs:(NSString *)vfsName {
    ...
    if (self != nil) {
        ...
        _databaseInPool     = AMAFMDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = AMAFMDBReturnRetained([NSMutableArray array]);
...
- (void)dealloc {
    ...
    AMAFMDBRelease(_databaseInPool);
    AMAFMDBRelease(_databaseOutPool);
    AMAFMDBRelease(_vfsName);
    
    if (_lockQueue) {
        AMAFMDBDispatchQueueRelease(_lockQueue);
        _lockQueue = 0x00;
    }
...
- (void)beginTransaction:(FMDBTransaction)transaction withBlock:(void (^)(AMAFMDatabase *db, BOOL *rollback))block {
...
- (void)inImmediateTransaction:(__attribute__((noescape)) void (^)(AMAFMDatabase *db, BOOL *rollback))block {
