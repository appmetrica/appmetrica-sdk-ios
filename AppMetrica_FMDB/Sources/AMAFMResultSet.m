...
+ (instancetype)resultSetWithStatement:(AMAFMStatement *)statement usingParentDatabase:(AMAFMDatabase*)aDB shouldAutoClose:(BOOL)shouldAutoClose {
    AMAFMResultSet *rs = [[AMAFMResultSet alloc] init];
    ...
    return AMAFMDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    AMAFMDBRelease(_columnNameToIndexMap);
...
- (NSDictionary *)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return AMAFMDBReturnAutoreleased([dict copy]);
    }
    else {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"AMAFMDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
...
- (AMASqliteValueType)typeForColumn:(NSString*)columnName {
    return sqlite3_column_type([_statement statement], [self columnIndexForName:columnName]);
}

- (AMASqliteValueType)typeForColumnIndex:(int)columnIdx {
